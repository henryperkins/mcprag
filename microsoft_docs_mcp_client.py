...
class MicrosoftDocsMCPClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            response = await self._send_request(
                {
                    ...
                    "params": {
                        ...
                        "capabilities": {"tools": {}},
                        "clientInfo": {"name": "mcp-rag-client", "version": "1.0.0"},
                    },
                    "id": 1,
                }
            )
            ...
            logger.info(
                f"Available Microsoft Docs tools: {[t['name'] for t in self.tools]}"
            )
    ...
    async def _send_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Sending request to {self.base_url}: {json.dumps(request, indent=2)}"
        )
        ...
        try:
            async with self.session.post(
                ...
                if "text/event-stream" in content_type:
                        ...
                        if line.startswith("data: "):
                            ...
                            if data:
                                try:
                                    ...
                                    logger.debug(
                                        f"Parsed SSE response: {json.dumps(parsed, indent=2)}"
                                    )
                                    return parsed
                                except json.JSONDecodeError as e:
                                    logger.debug(
                                        f"Failed to parse JSON from SSE line: {e}"
                                    )
                                    continue
                    ...
                    logger.debug(
                        f"Parsed JSON response: {json.dumps(parsed, indent=2)}"
                    )
                    return parsed
        ...
        for section in sections:
            if section.strip():
                ...
                if lines:
                    ...
                    results.append(
                        {"title": title, "content": content, "source": "Microsoft Docs"}
                    )
