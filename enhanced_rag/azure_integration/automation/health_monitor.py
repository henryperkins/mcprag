...
class HealthMonitor:
    ...
    def __init__(self, operations: SearchOperations, monitor: Optional[PerformanceMonitor] = None, debug: bool = False):
        """Initialize health monitor.
        
        Args:
            operations: SearchOperations instance
            monitor: Optional PerformanceMonitor to record timings
            debug: When True, include performance metrics in full report
        """
        self.ops = operations
    ...
    async def get_service_health(self) -> Dict[str, Any]:
        ...
        """
        cm = self.monitor.span("health.get_service_health") if getattr(self, "monitor", None) else nullcontext()
        with cm:
            health_status = {
                "status": "healthy",
    ...
    async def check_indexes_health(self) -> List[Dict[str, Any]]:
        ...
        """
        cm = self.monitor.span("health.check_indexes_health") if getattr(self, "monitor", None) else nullcontext()
        with cm:
            indexes_health = []
    ...
    async def check_indexers_health(self) -> List[Dict[str, Any]]:
        ...
        """
        cm = self.monitor.span("health.check_indexers_health") if getattr(self, "monitor", None) else nullcontext()
        with cm:
            indexers_health = []
    ...
    async def get_full_health_report(self) -> Dict[str, Any]:
        ...
        Returns:
            Full health report
        """
        cm = self.monitor.span("health.get_full_health_report") if getattr(self, "monitor", None) else nullcontext()
        with cm:
            ...
            if getattr(self, "debug", False) and getattr(self, "monitor", None):
                result["debug"] = self.monitor.get_metrics()
            return result