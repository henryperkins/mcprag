        ...
        try:
            ...
            if index.vector_search:
                ...
                if not vector_fields:
                    ...
                    # Get expected dimensions from config or embedding model
                    from enhanced_rag.core.config import get_config
                    config = get_config()
                    expected_dims = getattr(config.azure, "embedding_dimensions", 1536)

                    for vf in vector_fields:
                        if vf.name == "content_vector" and vf.vector_search_dimensions != expected_dims:
                            logger.warning(
                                f"Vector field 'content_vector' has {vf.vector_search_dimensions} dimensions, "
                                f"expected {expected_dims} for current embedding model"
                            )
                            dimension_mismatches.append({
                                "field": vf.name,
                                "current": vf.vector_search_dimensions,
