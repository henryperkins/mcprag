...
def check_prerequisites():
    ...
    if not Path(".env").exists():
        issues.append(
            "❌ .env file not found. Create one with ACS_ENDPOINT and ACS_ADMIN_KEY"
        )
    else:
        load_dotenv()
    ...
    try:
        ...
        issues.append(
            "❌ Azure Search Documents not installed. Run: pip install -r requirements.txt"
        )
...
def show_repository_stats():
    ...
    print(
        f"  📄 Total indexable files: {len(python_files) + len(js_files) + len(ts_files)}"
    )

    if python_files:
        print(
            f"  🐍 Sample Python files: {', '.join(str(f) for f in python_files[:3])}"
        )
    if js_files:
        print(
            f"  🟨 Sample JavaScript files: {', '.join(str(f) for f in js_files[:3])}"
        )
    if ts_files:
        print(
            f"  🔷 Sample TypeScript files: {', '.join(str(f) for f in ts_files[:3])}"
        )
