...
def get_resource_info():
    ...
    if Path(".env").exists():
        ...
        if endpoint:
            ...
            service_name = endpoint.replace("https://", "").replace(
                ".search.windows.net", ""
            )
...
def cleanup_local_files():
    ...
    directories_to_remove = ["__pycache__", ".pytest_cache", "logs", "temp"]
...
def stop_running_processes():
    ...
    health_check = run_command(
        "curl -s http://localhost:8001/health", ignore_errors=True
    )
...
def main():
    ...
    if azure_available:
        ...
        delete_search_index(
            resource_info["search_service"], admin_key, resource_info["index_name"]
        )

        # Step 5: Delete search service
        delete_search_service(
            resource_info["search_service"], resource_info["resource_group"]
        )
