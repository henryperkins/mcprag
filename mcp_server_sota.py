...
class MCPServer:
    def __init__(self):
        ...
        if not acs_endpoint:
            raise ValueError("ACS_ENDPOINT environment variable is not set")
        if not acs_admin_key:
            raise ValueError("ACS_ADMIN_KEY environment variable is not set")
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "azure-code-search", "version": "1.0.0"},
                },
            }
    ...
    async def handle_tool_call(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tool_name == "search_code":
            ...
            try:
                if MicrosoftDocsMCPClient is None:
                    raise ImportError("MicrosoftDocsMCPClient is not available")
        ...
        if self.embedder:
            ...
            if query_embedding:
                vector_query = VectorizedQuery(
                    vector=query_embedding, k_nearest_neighbors=50, fields="code_vector"
                )
...
class MCPSearchRequest(BaseModel):
    ...
    context: Optional[Dict] = Field(
        None, description="Current code context from Claude"
    )
    intent: Optional[str] = Field(
        None, description="Search intent: implement/debug/understand/refactor"
    )
    repository: Optional[str] = Field(
        None, description="Repository to search (defaults to current, use '*' for all)"
    )
...
async def mcp_contextual_search(request: MCPSearchRequest):
    ...
    try:
        results = await server.search_code_enhanced(
            query=request.input, intent=request.intent, repository=request.repository
        )
