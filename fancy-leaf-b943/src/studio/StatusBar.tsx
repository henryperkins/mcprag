...
export default function StatusBar({ messages }: { messages: Message[] }) {
  ...
  const turns = React.useMemo(() => {
    const fromMessages = messages.filter(m => m.role === 'user' || m.role === 'assistant').length
    return session?.stats?.numTurns ?? fromMessages
  }, [messages, session])
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-2 text-xs text-[color:var(--text-muted)] border-t border-[color:var(--border-subtle)] bg-[color:color-mix(in srgb, var(--bg-primary) 85%, transparent)]">
      <div className="flex items-center gap-5">
        ...
        <span className="inline-flex items-center gap-1.5"><Coins className="h-3.5 w-3.5 opacity-80" /> {session?.stats?.totalCostUsd ? `$${session.stats.totalCostUsd.toFixed(4)}` : `~${tokens.toLocaleString()} tok`}</span>
        <span className="inline-flex items-center gap-1.5"><Timer className="h-3.5 w-3.5 opacity-80" /> {session?.stats?.durationMs ? `${session.stats.durationMs} ms` : 'live'}</span>
      </div>
    </div>
  )
}
